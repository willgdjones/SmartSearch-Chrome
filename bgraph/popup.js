...
resize = function()
{
    ...
    view.width = window.innerWidth-20;
    view.height = window.innerHeight-20;
//    console.log(window);
}
...
var DataReader = (function ()
{
    ...
    o.GetChildren = function ( pageId )
    {
        ...
        if( !localStorage[ "children\\\\" + pageId ] )
        {
            return []
        }
    ...
    o.GetPage = function ( pageId )
    {
        ...
        o.id = pageId
        o.url = localStorage[ "url\\\\" + pageId ] ? localStorage[
            "url\\\\" + pageId ] : ''
        o.title = localStorage[ "title\\\\" + pageId ] ? localStorage[
            "title\\\\" + pageId ] : ''
        o.date = localStorage[ "date\\\\" + pageId ] ? localStorage[
            "date\\\\" + pageId ] : ''
        o.image = localStorage[ "image\\\\" + pageId ] ? localStorage[
            "image\\\\" + pageId ] : ''
        o.parent = localStorage[ "parent\\\\" + pageId ] ? parseInt(
            localStorage[ "parent\\\\" + pageId ] ) : 0
        o.dirtyurl = localStorage[ "dirtyurl\\\\" + pageId ] ? localStorage[ "dirtyurl\\\\" + pageId ] : ''
    ...
    o.GetMaxId = function ()
    {
        return localStorage['changeCount'] ? parseInt(
            localStorage['changeCount'] ) : 0
    }
...
var Graph = (function ()
{
    ...
    o.AddPage = function ( pageData )
    {
        if( pageData.parent )
        {
			...
			var pageData2 = pageData
			
			while( pageData2.parent )
			{
				pageData2 = pages[ pageData2.parent ]
				parentCount ++
			}
            ...
            pageData.r = 10
			pageData.rr = 60-10*parentCount;
            pageData.hover = false
        }
        else
        {
            pageData.x = Math.random() * 100 - 50
            pageData.y = Math.random() * 100 - 50
            pageData.r = 10
			pageData.rr = 60;
            pageData.hover = false
		...
		{
			case 0:
				pageData.image.src = 'site-facebook.png'
				break;
			case 1:
				pageData.image.src = 'site-news.png'
				break;
			case 2:
				pageData.image.src = 'site-google.png'
				break;
			case 3:
				pageData.image.src = 'site-bbc.png'
				break;
			default:
				pageData.image.src = 'site-ibuddy.png'
				break;
		}
    ...
    var update = function ()
    {
        ...
        for( i = 0; i < pageIds.length; i++ ) if( pageIds[i] != draggingPageId )
        {
            ...
            for( j = 0; j < pageIds.length; j++ ) if( i != j )
            {
                ...
                var rr = pageData1.rr + pageData2.rr
                var f = 0

                var dist = pageData1.parent == pageIds[j] || pageData2.parent ==
                                                             pageIds[i] ? 40 : 140
            ...
            //  Attract towards center
                      ...
                      pageData1.x = pageData1.x*0.99;
                      pageData1.y = pageData1.y*0.99;

            //  Change radius based on hover

            if( pageData1.hover )
            {
                pageData1.r = (pageData1.rr + 20 + pageData1.r) * 0.5
            }
            else
            {
                pageData1.r = (pageData1.rr + pageData1.r) * 0.5
            }
        ...
        for( i = 0; i < pageIds.length; i++ )
        {
            ...
            im.fillStyle = '#3b5999';
            im.beginPath();
            im.arc( cx + pageData1.x, cy + pageData1.y, pageData1.r, 0,
                    Math.PI * 2, true );
            im.closePath();
            im.fill();
            ...
            im.fillStyle = '#ffffff';
            im.beginPath();
            im.arc( cx + pageData1.x, cy + pageData1.y, pageData1.r - 8, 0,
                    Math.PI * 2, true );
            im.closePath();
            im.fill();
			...
			im.save();
				...
				im.beginPath();
            im.arc( cx + pageData1.x, cy + pageData1.y, pageData1.r - 12, 0,
                    Math.PI * 2, true );
				im.closePath();
				im.clip();

            im.drawImage( pageData1.image, cx + pageData1.x-132/2, cy + pageData1.y-212/2);
        ...
        if( hoverPageId )
        {
            ...
            var h = popupImg.height;
            canvas.getContext('2d').drawImage(popupImg, mouseX+cx-(w/2), mouseY+cy-h-10);

            im.fillStyle = '#000000'

			//  draw title and description
			
            pageData = pages[ hoverPageId ];

            im.font = "20pt Arial";
            title = pageData.title.substr(0, 20);
            if( pageData.title.length > 20 )
            {
                title = title + "...";
            }
            im.fillText( title, mouseX+cx-(w/2)+20, mouseY+cy-(h/2)-35 );
            im.font = "15pt Arial";

            url = pageData.url.substr(0, 35);
            if( pageData.url.length > 35 )
            {
                url = url + "...";
            }
            im.fillText( url, mouseX+cx-(w/2)+20, mouseY+cy-(h/2)-5 );
        }
        else
        {
    ...
    {
        if( !dragged && hoverPageId )
        {
            var pageData = pages[ hoverPageId ]
            chrome.tabs.create( {url:pageData.url } )
        }
...
function processQueue()
{
    ...
    if( newMaxId > lastMaxId )
    {
        for( var i = lastMaxId + 1; i <= newMaxId; i++ )
        {
            ...
            if( pageData.parent == 0 || pageData.parent <= lastMaxId )
            {
                queue.push( i )
            }
        }
